						...
						<div class="tabs-box">
                            ...
                            <div class="widget-content">
                                <div class="table-outer">
											...
											@else
                                                @foreach ($jobseekers as $jobseeker)
                                                    <tr>
                                                        <td>
                                                            {{ $jobseeker->name }}
                                                            <a href="{{ route('pages.candidate.detail', ['uniqueId' => $jobseeker->candidateInfo->unique_id ]) }}">
                                                        ...
                                                        <td>{{ $jobseeker->email_verified_at }}</td>
                                                        <td>
                                                            <div class="ls-show-more m-0">
                                                                <div class="bar m-0" style="width: 100px">
                                                                    <span class="bar-inner" style="width: {{ $jobseeker->isComplete }}%"></span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            @if ($jobseeker->status === 'active')
                                                                <a href="#" data-toggle="tooltip" data-placement="bottom" title="Deactivate Account" 
                                                                    onclick="handleUserStatus(event, '{{ openssl_encrypt($jobseeker->id, 'AES-128-ECB', 'FP25Hg9KKNJx') }}', 'disabled')"
                                                                >
                                                                    <i class="la la-user-slash f2x ui-red"></i>
                                                                </a>
                                                            @else
                                                                <a href="#" data-toggle="tooltip" data-placement="bottom" title="Activate Account" 
                                                                    onclick="handleUserStatus(event, '{{ openssl_encrypt($jobseeker->id, 'AES-128-ECB', 'FP25Hg9KKNJx') }}', 'active')"
                                                                >
                                                                    <i class="la la-user-check f2x ui-green"></i>
                                                                </a>
                                                            @endif
                                                            <a href="#" data-toggle="tooltip" data-placement="bottom" title="Delete Account"
                                                                onclick="handleMemberDelete(event, '{{ openssl_encrypt($jobseeker->id, 'AES-128-ECB', 'FP25Hg9KKNJx') }}')"
                                                            >
                                    ...
                                    <div class="mb-3">
                                        {{ $jobseekers->links('pages.pagination') }}
									</div>
                                    <!-- Listing Show More -->
                                    @if ($jobseekers->hasPages())
                                        <div class="ls-show-more mt-0">
                                            <p>Showing {{ number_format($jobseekers->lastItem()) }} of {{ number_format($jobseekers->total()) }} Jobseekers</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
