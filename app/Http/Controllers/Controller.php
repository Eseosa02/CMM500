...
class Controller extends BaseController
{
    ...
    public function dashboardRedirection() {
        if (Auth::user()->role === User::ROLE_CANDIDATE) {
            return redirect()->route('dashboard.candidate.index');
        }

        if (Auth::user()->role === User::ROLE_EMPLOYER) {
            return redirect()->route('dashboard.employer.index');
        }
    }

    public function generateUniqueId($model) {
        $uniqueId = $this->generateRandomString();
        $result = null;
        if ($model === User::ROLE_EMPLOYER) {
            $result = EmployerProfile::where(EmployerProfile::UNIQUE_ID, $uniqueId)->first();
        } else if ($model === User::ROLE_CANDIDATE) {
            $result = UserProfile::where(UserProfile::UNIQUE_ID, $uniqueId)->first();
        } else {
            $result = JobListing::where(JobListing::JOB_REFERENCE, $uniqueId)->first();
        }

        if ($result) {
            return $this->generateUniqueId($model);
        }
    ...
    public function getStarApplicants($applications, $listing) {
        $listingSkills = $listing->skills;
        $ratingArr = array();

        foreach ($applications as $applicant) {
            $applicantSkill = $applicant->candidateInfo->rating;
            $total = 0;
            foreach($applicantSkill as $skill => $value) {
                if (in_array($skill, $listingSkills)) {
                    $total += $value;
                }
            }
            if ($total > 0) {
                array_push($ratingArr, ["applicationId" => $applicant->id, "rating" => $total]);
            }
        }

        array_multisort(array_column($ratingArr, 'rating'), SORT_DESC, $ratingArr);
        return array_column(array_slice($ratingArr, 0, 3), 'applicationId');
    }
}
